from pathlib import Path

from backend.services.parsers import parse_gedmatch


def to_alleles(genotype: str | None) -> list[str] | None:
    if genotype is None:
        return None
    return [allele for allele in genotype]


def load_lines(path: str) -> list[str]:
    sample_path = Path(__file__).resolve().parents[2] / path
    with open(sample_path, "r", encoding="utf-8", errors="ignore") as f:
        return f.readlines()


def test_parse_gedmatch_edge_file() -> None:
    lines = load_lines("dna_samples/gedmatch/gedmatch_edge.txt")
    result = parse_gedmatch(lines)

    # All structurally valid → all kept
    assert len(result.variants) == 7

    # Three invalid genotypes → 3 errors
    assert len(result.errors) == 3

    # Spot checks
    assert result.variants[0].rsid == "."  # missing rsid allowed
    assert result.variants[2].genotype == ["C"]  # haploid
    assert result.variants[3].genotype == ["I", "D"]  # indel normalization
    assert result.variants[4].genotype is None  # missing genotype


def test_parse_gedmatch_messy_file() -> None:
    lines = load_lines("dna_samples/gedmatch/gedmatch_messy.txt")
    result = parse_gedmatch(lines)

    print(result.variants)
    print(result.errors)
    assert len(result.variants) == 6
    assert len(result.errors) == 3

    # normalization
    assert result.variants[0].genotype == ["A", "A"]

    # invalid genotype handled
    assert any(v.genotype is None for v in result.variants)

    # ensure garbage line dropped
    assert all(v.rsid != "bad" for v in result.variants)


def test_parse_gedmatch_valid_file() -> None:
    lines = load_lines("dna_samples/gedmatch/gedmatch_valid.txt")
    result = parse_gedmatch(lines)

    assert len(result.variants) == 4
    assert result.errors == []

    # correctness
    assert result.variants[0].genotype == ["A", "A"]
    assert result.variants[-1].chromosome == "7"


def test_parse_gedmatch_broken_file() -> None:
    lines = load_lines("dna_samples/gedmatch/gedmatch_broken.txt")
    result = parse_gedmatch(lines)

    assert len(result.variants) == 0


def test_parse_gedmatch_empty_file() -> None:
    result = parse_gedmatch([])

    assert result.variants == []
